...
export DATA_ROOT="./outputs/LFGC"
export TRAINER=${TRAINER:-LFGCTrainer}
export INLIER_MODEL=${INLIER_MODEL:-LFGCNet}
...
export COLLATION_2D=${COLLATION_2D:-collate_lfgc}
export BEST_VAL_METRIC=${BEST_VAL_METRIC:-f1}
export TIME=$(date +"%Y-%m-%d_%H-%M-%S")
export VERSION=$(git rev-parse HEAD)

export OUT_DIR=${DATA_ROOT}/${DATASET}-i${INLIER_THRESHOLD_PIXEL}/${INLIER_MODEL}-${BEST_VAL_METRIC}/${OPTIMIZER}-lr${LR}-e${MAX_EPOCH}-b${BATCH_SIZE}/${PATH_POSTFIX}/${TIME}
